...
from engine.value import _rank_movers, compute_values
...
def _load_players(args: argparse.Namespace) -> tuple[dict, str]:
    ...
    if args.actuals:
        ...
        return players, f"actuals: {season}"
...
def _board_rows(players: dict, cfg: LeagueConfig, basis: str) -> list[dict]:
    ...
    adj_board = compute_values(players, cfg, basis="availability_adjusted")
    pg_board = compute_values(players, cfg, basis="per_game")
...
_CSV_FIELDS = ("rank", "name", "nba_id", "value", "vor",
               "availability_adjusted_rank", "per_game_rank", "rank_delta")


def _print_table(rows: list[dict], league_name: str, description: str, basis: str, top: int) -> None:
    print(f"\n{league_name}  ({description})  basis={basis}")
    print(f"{'#':>4} {'player':<26} {'value':>8} {'vor':>7} {'adj#':>6} {'pg#':>6} {'delta':>6}")
    print("-" * 68)
    for row in rows[:top]:
        print(f"{row['rank']:>4} {(row['name'] or '?'):<26} {row['value']:>8} {row['vor']:>7} "
...
def cmd_explain(args: argparse.Namespace, players: dict, cfg: LeagueConfig) -> int:
    explain(players, cfg, args.explain, basis=args.basis)
    return 0
...
def cmd_compare(args: argparse.Namespace, players: dict) -> int:
    ...
    board_a = compute_values(players, cfg_a, basis=args.basis)
    board_b = compute_values(players, cfg_b, basis=args.basis)
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        rows = _board_rows(players, cfg, args.basis)
        _print_table(rows, cfg.name, description, args.basis, args.top)
        if args.csv:
